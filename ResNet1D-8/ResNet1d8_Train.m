...
% @Function:8 layer residual network training program
...
NUM=500;   %Number of test samples
dw=6;  %Number of resistivity
t=60;  %Number of frequencies
C=60;  %Interpolation length
number_matrix=zeros(num,dw-1);  %Interpolation position
T=logspace(0,4,t)';  %period
h=20+10.^(0.0295*(0:t-2)); %sixty layer
w=zeros(num,C);  %Resistivity sample
apparentResistivity=zeros(t,num);  %apparent resistivity
phase=zeros(t,num);  %phase position
B = repmat(T,1,num);
...
load ResNet1d8_Layers.mat %import Network layer
% Set sample size
[m,n]=size(apparentResistivity);
num=n;  %Total number of samples
NUM=100;   %Number of test samples
% Normalized apparent resistivity data
app_log=log10(apparentResistivity); %Logarithmic normalization
max_input=max(max(app_log));        %take the maximum
app_a=app_log/max_input;            %mapping（0，1]
% The resistivity data is normalized 
w_log=log10(w);                     %Logarithmic normalization
max_output=max(max(w_log));         %take the maximum
w_a=w_log/max_output;               %mapping（0，1]
% Assigned training sample
app_a_train=app_a(:,1:num-NUM);     %Take the apparent resistivity training data
app_a_test=app_a(:,num-NUM+1:num);  %Take the resistivity test data
% Assigned test sample
w_a_train=w_a(:,1:num-NUM);         %Take the apparent resistivity training data
w_a_test=w_a(:,num-NUM+1:num);      %Take the resistivity test data
...
[resnet1d8net_500K,resnet1d8info_500K] = trainNetwork(XTrain,YTrain,layers,options);
